...
@property (nonatomic,strong) BJGameModel *gameModel;
...
-(id) initWithCoder:(NSCoder *)aDecoder{
    ...
    /*if(self){
        self.gameModel = [[BJGameModel alloc] init];
...
-(void) showNextDealerCard{
    BJCard *card = [self.gameModel nextDealerCard];
    card.isFaceUp = YES;
    [self renderCards];
...
-(void)showSecondDealerCard{
    BJCard *card=[self.gameModel lastDealerCard];
    card.isFaceUp=YES;
    [self renderCards];
    [self.gameModel updateGameStage];
    if(self.gameModel.gameStage!=BJGameStageGameOver){
...
- (IBAction)didTapHitButton:(id)sender {
    BJCard *card=[self.gameModel nextPlayerCard];
    card.isFaceUp=YES;
    [self renderCards];
